...
export function createQuestionnaireRepository(
   ...
   */
  const getPoguesQuestionnaire = useConstCallback(
    (poguesId: string): Promise<Questionnaire> => {
   ...
   */
  const getQuestionnaireFromPoguesId = useConstCallback(
    (poguesId: string): Promise<Questionnaire> => {
      return getRequest<Questionnaire>(
        `${apiUrl}/questionnaires/${poguesId}/db`
      )(token);
    }
  );

  const addQuestionnaire = useConstCallback(
    (questionnaire: Questionnaire): Promise<Questionnaire> => {
  ...
  const editQuestionnaire = useConstCallback(
    (questionnaire: Questionnaire): Promise<Questionnaire> => {
      const formData = new FormData();
  ...
  const deleteQuestionnaire = useConstCallback((id: number): Promise<void> => {
    return deleteRequest<void>(`${apiUrl}/questionnaires/${id}/delete`)(token);
  });

  const getSurveyContexts = useConstCallback((): Promise<SurveyContext[]> => {
